...
from rule_based_classifier import RuleBasedClassifier, ClassifierConfig
...
def main():
    ...
    clf = RuleBasedClassifier(ClassifierConfig())
    ...
    with mp_holistic.Holistic(
        ...
        while cap.isOpened():
            ...
            # curl 디버그 표시
            draw_debug(image, ff)
            if label not in ("HOLD", "UNKNOWN"):
                print(label, flush=True)
            cv2.imshow("Emotion Gestures (JOY / SAD / ANGER / DISLIKE / NEUTRAL)", image)
